class Image < ApplicationRecord
  belongs_to :theme
  scope :theme_images, -> (theme_id) { select('id','name','file','av_value').where(theme_id: theme_id) }
  ...
  def self.update_ave_value(image_id, ave_value)
    image = find(image_id)
    image.update(av_value: ave_value)
  end
  ...
  def self.show_valued_image(new_value_data)
    ...
    if user_valued == 1
      common_ave_value = find(image_id).av_value.round
    else
      common_ave_value = 0
    end
